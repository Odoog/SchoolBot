...
<?php
	...
	class TelegramBot{ 
		protected $token = "634145150:AAFjB-RXGBXDZJOsB07LqLB2IqB52_KnLFc";
		protected $proxy = "tcp://195.235.204.60:3128";
		...
		protected function query($method, $params = []){
			try {
				...
				$client = new Client([
					'base_uri' => $url,
					'proxy' => [
        				'http'  => $this->proxy, // Use this proxy with "http"
        				'https' => $this->proxy, // Use this proxy with "https",
    				]
				]);
		...
		public function sendMessage($chat_id, $text, $answers = NULL){
			...
			if($answers){
				//debug("#answers" , json_encode($answers));
					$keyboard = array(
						"keyboard" => $answers,
						"one_time_keyboard" => false, // можно заменить на FALSE,клавиатура скроется после нажатия кнопки автоматически при True
						"resize_keyboard" => true // можно заменить на FALSE, клавиатура будет использовать компактный размер автоматически при True
